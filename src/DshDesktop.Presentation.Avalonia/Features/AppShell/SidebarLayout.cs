...
/// <summary>
/// 表示侧栏折叠的布局规则（视觉基准 docs/DSH-Desktop-UI-Prototype.html 第 64 行
/// 的 <c>@media(max-width:900px)</c> 降级规则，Phase 8 后续补充为可手动切换）。
/// 规则集中在此以便单测，MainWindow 只按结果消费，不重复硬编码。
/// </summary>
public static class SidebarLayout
{
    /// <summary>
    /// 侧栏展开宽度（原型 <c>.shell grid-template-columns:218px</c>）。
    /// </summary>
    public const double ExpandedWidth = 218d;

    /// <summary>
    /// 侧栏折叠宽度（原型折叠态 <c>grid-template-columns:70px</c>）。
    /// </summary>
    public const double CollapsedWidth = 70d;
